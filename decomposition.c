...
Domain* initDecomposition(int xproc, int yproc, int zproc, real3 globalExtent, Context *context)
{
   assert( xproc * yproc * zproc == getNRanks());

   printf("Inside initDecomposition\n");

 //  Domain* dd = comdMalloc(sizeof(Domain));
   Domain* dd;
   cncHandle_t d_handle = cncCreateItem_DD(&dd,sizeof(Domain));
   cncPut_DD(d_handle, 1, context);
