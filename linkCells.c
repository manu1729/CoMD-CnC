...
LinkCell* initLinkCells(const Domain* domain, real_t cutoff, Context *context)
{
   assert(domain);
//   LinkCell* ll = comdMalloc(sizeof(LinkCell));

   LinkCell *ll;
   cncHandle_t ll_handle = cncCreateItem_LC(&ll,sizeof(LinkCell));
   cncPut_LC(ll_handle, 1, context);
...
//   cncHandle_t na_handle = cncCreateItem_NAtoms(&ll->nAtoms,ll->nTotalBoxes*sizeof(int));
//   cncPut_NAtoms(na_handle, 1, context);
