...
Atoms* initAtoms(LinkCell* boxes, Context *context)
{
  // Atoms* atoms = comdMalloc(sizeof(Atoms));
   ...
   /*
   cncHandle_t gid_handle = cncCreateItem_GID(&atoms->gid,maxTotalAtoms*sizeof(int));
   cncPut_GID(gid_handle, 1, context);

   cncHandle_t isp_handle = cncCreateItem_ISP(&atoms->iSpecies,maxTotalAtoms*sizeof(int));
   cncPut_ISP(isp_handle, 1, context);

   cncHandle_t r_handle = cncCreateItem_R(&atoms->r,maxTotalAtoms*sizeof(real3));
   cncPut_R(r_handle, 1, context);

   cncHandle_t p_handle = cncCreateItem_P(&atoms->p,maxTotalAtoms*sizeof(real3));
   cncPut_P(p_handle, 1, context);

   cncHandle_t f_handle = cncCreateItem_F(&atoms->f,maxTotalAtoms*sizeof(real3));
   cncPut_F(f_handle, 1, context);

   cncHandle_t u_handle = cncCreateItem_U(&atoms->U,maxTotalAtoms*sizeof(real_t));
   cncPut_U(u_handle, 1, context);
*/
   atoms->nLocal = 0;
   atoms->nGlobal = 0;
