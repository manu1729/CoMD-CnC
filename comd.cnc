...
[ B : i, 0, 0, iter ], [ ATOMS : i,0,0,iter] -> ( advanceVelocityStep : i, iter ) -> [ B : i, 1, 0, iter], [ ATOMS : i,1,0,iter];
( advanceVelocityStep : i, iter ) -> < UpdateBox : 0, 0, iter > ; // generated when i == 0, remaning UpdateBox tags are generated by updateBoxStep

[ B : i, 1, 0, iter ], [ ATOMS : i,1,0,iter]-> ( updateBoxStep : i, k, iter ) -> < UpdateNbrs : i, j, iter > ; // only tag for the first neighbor step is generated here 
( updateBoxStep : i, k, iter ) -> [ AtomInfo : i, j, iter ];
( updateBoxStep : i, k, iter ) -> < GenForceDataTag : 0, iter>;  // generated by the last box

[ AtomInfo : i, j, iter ], [ B : j, 1, 0, iter ], [ ATOMS : j,1,0,iter] -> ( updateNeighborsStep : i, j, iter ) -> < UpdateNbrs : i, k , iter >; // generate tag for the next neighbor step
...
[ B : i, 1, 0, iter ], [ ATOMS : i,1,0,iter]-> (generateDataforForceStep: i, iter) -> [ B : i, 3, 0, iter ], [ ATOMS : i,3,0,iter], < GenForceDataTag : i+1, iter>;
...
[ B : i, 3, 0, iter ], [ ATOMS : i,3,0,iter] -> ( forceStep : i, iter ) -> < ForcefromNbrs : i, j, k, iter >; // generate tag for force computation due to the first neighbor 
[ B : i, 3, k, iter ], [ B : j, 3, 0, iter ], [ ATOMS : i,3,k,iter], [ ATOMS : j,3,0,iter] -> ( computeForcefromNeighborsStep: i, j, k, iter ) -> [ B : i, 3, k+1, iter ], [ ATOMS : i,3,k+1,iter], < ForcefromNbrs : i, jnext, k+1, iter >;
( computeForcefromNeighborsStep: i, j, k, iter ) -> [ B : i, 4, 0, iter ], [ ATOMS : i,4,0,iter]; // when k == 27-1


[ B : i, 4, 0, iter ],[ ATOMS : i,4,0,iter], [redc : i, iter ], [ IT : 0 ], [ TBoxes : 0 ] -> ( reduceStep : i, iter ) -> [ redc : i+1, iter ], [ B : i, 0, 0, iter+1 ], [ ATOMS : i,0,0,iter+1] ;
( reduceStep : i, iter ) -> [ B : i, 5, 0, iter ], [ ATOMS : i,5,0,iter] ; // is executed only when i == 1727 and iter == max_iteration
( reduceStep : i, iter ) -> [ redc : 0, iter+1 ] ;  // when i = TBoxes[0] and iter < IT[0]


// Write graph inputs and start steps
env -> [ B : i, 0, 0, 1 ], [ ATOMS : i,0,0,1], [ s : 0, 1 ], [ IT : 0 ], [ TBoxes : 0 ], [ Nbs : 0 ]; //[ TAtoms : 0 ], [ AperB : 0 ], 
...
[ B : i, 5, 0, iter  ], [ ATOMS : i,5,0,iter], [ redc : i+1, iter] -> ( env: i, iter );
